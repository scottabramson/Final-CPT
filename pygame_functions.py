...
key_states = {key: False for key in keydict.values()}
...
def moveSprite(sprite, x, y, centre=False): # delete sprite 2
...
def showSprite(sprite): #delete sprite 2
...
def keyPressed(keyCheck=""):
    ...
    if sum(keys) > 0:
        if keyCheck == "" or keys[keydict[keyCheck.lower()]]:
            return True
    return False
def keyReleased(keyCheck=""):
    global key_states
    if keyCheck.lower() in keydict:
        key = keydict[keyCheck.lower()]
        if key_states[key] and not pygame.key.get_pressed()[key]:
